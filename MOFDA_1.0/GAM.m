...
%[x,Fval,exitFlag,Output] = gamultiobj(MultiObj.fun,MultiObj.nVar ,A, ...
 %   b,Aeq,beq, MultiObj.var_min, MultiObj.var_max,options);

problem.options = optimoptions('paretosearch','PlotFcn',@psplotparetof);
[x,Fval,exitFlag,Output] = paretosearch(FitnessFunction,numberOfVariables,A,b,Aeq,beq,MultiObj.var_min,MultiObj.var_max,problem.options);

for i=1:size(Fval);
    pareto_sum(1,i) = sum(Fval(i,:));
end
% Display info

[fitb, indexes] = sort(pareto_sum);
display(['Best solution is :', num2str(Fval(indexes(1),:))]);
