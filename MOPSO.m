...
% ----------------------------------------------------------------------- %
function [REP, ConvergenceCurve] = MOPSO(params,MultiObj)
    ...
    ConvergenceCurve=inf*ones(2,Nr);
    pareto = inf*ones(2,Nr);
    VEL = zeros(Np,nVar);
    POS_fit  = fun(POS);
    ...
    if(size(POS_fit,2)==3)
        ...
        grid on; xlabel('f1'); ylabel('f2'); zlabel('f3'); %title('Schaffer');legend('MOPSO');
        drawnow;
        axis square;
    end
    ...
    while ~stopCondition
        ...
        if(size(POS_fit,2)==2)
            ...
            grid on; xlabel('f1'); ylabel('f2'); title('Viennet3');legend('MOPSO');
            drawnow;
            axis square;
        end
        if(size(POS_fit,2)==3)
            ...
            grid on; xlabel('f1'); ylabel('f2'); zlabel('f3'); title('Viennet2');legend('MOPSO');
            drawnow;
            axis square;
        end
